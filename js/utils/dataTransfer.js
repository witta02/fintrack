...
export function exportData() {
  ...
  alerts.success(t("exportSuccessTitle"), t("exportSuccessDesc"));
}
...
function applyImport(data, mode) {
  ...
  if (mode === "replace") {
    ...
    const newRules = incoming.recurringRules.filter(
  ...
  document.documentElement.setAttribute(
    "data-theme",
    store.settings.isDarkMode ? "dark" : "light",
  );
  document.documentElement.lang =
    store.settings.language === "en" ? "en" : "th";
...
function deflateToCompact(payload) {
  return {
    ...
    rr: (payload.data.recurringRules || []).map((r) => [
      ...
      r.customDays || 30,
      r.nextDueDate instanceof Date
        ? r.nextDueDate.getTime()
        : new Date(r.nextDueDate).getTime(),
      r.isActive ? 1 : 0,
...
export async function importFromText() {
  ...
  const { value: pastedText } = await Swal.fire({
    ...
    inputPlaceholder:
      lang === "en"
        ? "Paste your backup code here..."
        : "วางรหัสสำรองข้อมูลที่คัดลอกมาที่นี่...",
    inputAttributes: {
      autocorrect: "off",
      autocapitalize: "off",
      spellcheck: "false",
...
export async function exportToCloud() {
  ...
  try {
    ...
    const response = await fetch(
      `https://ntfy.sh/fintrack-cloud-sync-${code}`,
      {
        method: "PUT",
        body: base64Text,
        headers: {
    ...
    if (!response.ok) {
      throw new Error(
        lang === "en"
          ...
          : "ไม่สามารถอัปโหลดข้อมูลสำรองได้",
      );
    }
...
export async function importFromCloud() {
  ...
  try {
    const res = await fetch(
      `https://ntfy.sh/fintrack-cloud-sync-${code}/json?poll=1`,
    );
    ...
    if (!base64Text) {
      throw new Error(
        lang === "en"
          ? "No backup found for this code."
          : "ไม่พบข้อมูลสำรองสำหรับรหัสนี้ หรือรหัสหมดอายุแล้ว",
      );
    }
    ...
    alerts.error(
      ...
      err.message ||
        (lang === "en"
          ? "Could not retrieve data. Please check the code."
          : "ไม่สามารถดึงข้อมูลได้ โปรดตรวจสอบรหัสเชื่อมโยงอีกครั้ง"),
    );
  }
}
