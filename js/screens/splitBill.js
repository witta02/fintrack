...
export function renderSplitBill(container) {
  ...
  container.innerHTML = `
    ...
    <div class="screen screen-enter" style="padding: 0 16px 100px; max-width: 520px; margin: 0 auto;">
      
      <!-- Header -->
      <div style="display: flex; align-items: center; gap: 10px; padding: 14px 0 18px;">
        <button id="split-back-btn" class="icon-btn" style="background: var(--surface); border: 1px solid var(--border); color: var(--text-primary); cursor: pointer; display: flex; align-items: center; justify-content: center; width: 38px; height: 38px; border-radius: var(--radius);">
      ...
      <!-- Result Double-Bezel Card -->
      <div class="bezel-card" style="margin-bottom: 16px;">
        <div class="bezel-inner" style="padding: 20px 16px; text-align: center;">
          <div style="font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.6px; color: var(--gold); margin-bottom: 4px;">
            ${language === 'en' ? 'Each Person Pays' : 'จ่ายคนละ (Per Person)'}
          </div>
          <div id="split-per-person-display" style="font-size: 38px; font-weight: 900; letter-spacing: -0.8px; color: var(--text-primary); font-family: var(--font-heading); line-height: 1.1; margin-bottom: 8px;">
            ${sym}0.00
          </div>
          <div id="split-summary-subtext" style="font-size: 11.5px; color: var(--text-secondary); font-weight: 600;">
      ...
      <div class="bezel-card" style="margin-bottom: 14px;">
        <div class="bezel-inner" style="padding: 16px;">
          <div class="split-label">
            <span>${language === 'en' ? 'Total Bill Amount' : 'ยอดเงินรวมทั้งหมด'}</span>
            <span style="font-size: 10.5px; color: var(--gold); font-weight: 800;">${store.getSelectedCurrency()}</span>
          </div>
          <div style="position: relative; margin-bottom: 10px;">
            <input 
              ...
              min="0" 
              placeholder="0.00"
              style="width: 100%; font-size: 24px; font-weight: 900; padding: 10px 14px; background: var(--input-bg); border: 1px solid var(--border); border-radius: var(--radius); color: var(--text-primary); font-family: var(--font-heading); outline: none;"
            />
          </div>
      ...
      <div class="bezel-card" style="margin-bottom: 14px;">
        <div class="bezel-inner" style="padding: 16px;">
          <div class="split-label">
            ...
            <span id="people-count-label" style="font-size: 11.5px; color: var(--gold); font-weight: 800;">2 ${language === 'en' ? 'people' : 'คน'}</span>
          </div>
          
          <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 10px;">
            <button id="minus-person-btn" class="stepper-btn" style="width: 38px; height: 38px; border-radius: var(--radius-sm);">−</button>
            <input 
              type="number" 
              ...
              max="100" 
              value="2" 
              style="flex: 1; text-align: center; font-size: 22px; font-weight: 900; padding: 8px; background: var(--input-bg); border: 1px solid var(--border); border-radius: var(--radius); color: var(--text-primary); font-family: var(--font-heading); outline: none;"
            />
            <button id="plus-person-btn" class="stepper-btn" style="width: 38px; height: 38px; border-radius: var(--radius-sm);">+</button>
          </div>
      ...
      <div class="bezel-card" style="margin-bottom: 16px;">
        <div class="bezel-inner" style="padding: 16px;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 10px;">
            <button id="toggle-service-btn" class="quick-chip" style="padding: 8px; text-align: center; border-radius: var(--radius-sm);">
              Service +10%
            </button>
            <button id="toggle-vat-btn" class="quick-chip" style="padding: 8px; text-align: center; border-radius: var(--radius-sm);">
              VAT +7%
            </button>
          </div>

          <div>
            <label style="display: block; font-size: 10.5px; font-weight: 700; color: var(--text-secondary); margin-bottom: 4px; text-transform: uppercase;">
            ...
            <input 
              ...
              step="0.01" 
              min="0" 
              style="width: 100%; font-size: 13px; padding: 10px 12px; background: var(--input-bg); border: 1px solid var(--border); border-radius: var(--radius); color: var(--text-primary); outline: none;"
            />
          </div>
        </div>
      </div>
      ...
      <div style="display: flex; flex-direction: column; gap: 8px; margin-bottom: 16px;">
        <button id="copy-summary-btn" class="btn-primary" style="padding: 14px; font-size: 13.5px; font-weight: 800; border-radius: var(--radius);">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#000" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
          <span>${language === 'en' ? 'Copy Share Message' : 'คัดลอกข้อความสรุปหารเงิน'}</span>
        </button>

        <button id="toggle-promptpay-btn" style="padding: 12px; font-size: 12.5px; font-weight: 700; border-radius: var(--radius); background: var(--surface); color: var(--text-primary); border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; gap: 8px; cursor: pointer;">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" stroke-width="2.2"><rect width="5" height="5" x="3" y="3" rx="1"/><rect width="5" height="5" x="16" y="3" rx="1"/><rect width="5" height="5" x="3" y="16" rx="1"/><path d="M21 16h-3a2 2 0 0 0-2 2v3"/><path d="M21 21v.01"/><path d="M12 7v3a2 2 0 0 1-2 2H7"/><path d="M3 12h.01"/><path d="M12 3h.01"/><path d="M12 16v.01"/><path d="M16 12h1"/><path d="M21 12v.01"/><path d="M12 21v-1"/></svg>
          <span>${language === 'en' ? 'Show PromptPay QR Code' : 'แสดง PromptPay QR สำหรับรับเงิน'}</span>
        </button>
      </div>

      <!-- PromptPay QR Double-Bezel Card (collapsible) -->
      <div id="promptpay-qr-section" class="bezel-card" style="display: none; margin-bottom: 16px;">
        <div class="bezel-inner" style="padding: 18px; text-align: center;">
          <h3 style="font-size: 13.5px; font-weight: 800; color: var(--text-primary); margin: 0 0 10px 0;">
