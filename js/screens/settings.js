    ...
    <div class="card" style="padding: 18px; margin-bottom: 20px; border: 1px solid var(--border); border-radius: 16px; background: var(--card);">
      <div style="display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 14px;">
        <div style="display: flex; align-items: center; gap: 12px;">
          <div class="setting-icon-badge" style="background: rgba(188, 140, 255, 0.12); width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; color: #BC8CFF;">
          ...
          <div style="text-align: left;">
            <div style="font-size: 13px; font-weight: 700; color: var(--text-primary);">\${store.settings.language === 'en' ? 'Custom Categories' : 'หมวดหมู่กำหนดเอง'}</div>
            <div style="font-size: 11px; color: var(--text-secondary); margin-top: 1px;">
              \${store.settings.language === 'en' ? 'Create your own transaction categories' : 'สร้างหมวดหมู่รายการของคุณเอง'}
            </div>
          </div>
        </div>
      </div>
      
      <div id="custom-categories-list" style="display: flex; flex-direction: column; gap: 8px; margin-bottom: 16px;">
        <!-- Dynamically rendered list of custom categories -->
      </div>

      <div style="display: flex; flex-direction: column; gap: 10px; padding-top: 14px; border-top: 1px solid var(--border);">
        <input type="text" id="new-cat-name" placeholder="\${store.settings.language === 'en' ? 'Category Name' : 'ชื่อหมวดหมู่'}" style="width: 100%; padding: 12px; border-radius: 12px; border: 1px solid var(--border); background: var(--surface); color: var(--text-primary); font-size: 14px;">
        ...
        <button id="add-custom-cat-btn" class="btn-primary" style="padding: 12px; font-size: 13px; font-weight: 700; border-radius: 12px; display: flex; align-items: center; justify-content: center; gap: 6px;">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
          \${store.settings.language === 'en' ? 'Add Category' : 'เพิ่มหมวดหมู่'}
        </button>
      </div>
    </div>
...
function setupEventListeners(container) {
  const renderCats = () => {
    const list = container.querySelector("#custom-categories-list");
    if (!list) return;
    list.innerHTML = "";
    const cats = store.settings.customCategories || [];
    if (cats.length === 0) {
      list.innerHTML = `<div style="font-size: 11px; color: var(--text-muted); text-align: center; padding: 8px 0;">${store.settings.language === 'en' ? 'No custom categories yet.' : 'ยังไม่มีหมวดหมู่ที่กำหนดเอง'}</div>`;
    } else {
      ...
      list.querySelectorAll(".delete-cat-btn").forEach(btn => {
        btn.addEventListener("click", async (e) => {
          ...
          const isConfirmed = await alerts.confirmDelete(
            store.settings.language === 'en' ? 'Delete Category?' : 'ลบหมวดหมู่?',
            store.settings.language === 'en' ? 'Are you sure you want to delete this custom category?' : 'คุณแน่ใจหรือไม่ที่จะลบหมวดหมู่นี้?'
          );
          if (isConfirmed) {
            cats.splice(idx, 1);
  ...
  if (container.querySelector("#custom-categories-list")) {
    renderCats();
    ...
    container.querySelector("#add-custom-cat-btn").addEventListener("click", () => {
      ...
      const emoji = container.querySelector("#new-cat-emoji").value.trim() || '📦';
      const color = container.querySelector("#new-cat-color").value;
      const type = container.querySelector("#new-cat-type").value;
      
      if (!name) {
        alerts.error(store.settings.language === 'en' ? 'Error' : 'ผิดพลาด', store.settings.language === 'en' ? 'Category name is required' : 'กรุณากรอกชื่อหมวดหมู่');
        return;
      }
      
      const cats = store.settings.customCategories || [];
      if (cats.some(c => c.name.toLowerCase() === name.toLowerCase())) {
        alerts.error(store.settings.language === 'en' ? 'Error' : 'ผิดพลาด', store.settings.language === 'en' ? 'Category already exists' : 'มีหมวดหมู่นี้อยู่แล้ว');
        return;
      }
      ...
      container.querySelector("#new-cat-name").value = '';
      container.querySelector("#new-cat-emoji").value = '';
      renderCats();
    });
  }
